...
class game():
    ...
    def play(self):
        ...
        while (not game_won):
            ...
            if (new_pos > 100):
                print(
                    f"You need exactly 100 to win, You stay at square {old_pos}\n")
                new_pos = old_pos

            if (new_pos in self.ladders):
                print(
                    f"You landed on a Ladder, it took you from square {new_pos} to {self.ladders[new_pos]}\n")
                new_pos = self.ladders[new_pos]

            if (new_pos in self.snakes):
                print(
                    f"You landed on a Snake, it took you from square {new_pos} to {self.snakes[new_pos]}\n")
                new_pos = self.snakes[new_pos]

            # update position after role
            self.player_pos[current_name] = new_pos
            ...
            if (new_pos == 100):  # win con
                ...
                # moves to the next player in the order
